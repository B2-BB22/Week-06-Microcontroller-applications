...
1. **Multiple Source Files**: เหตุใดต้องแยก source code เป็นหลายไฟล์? ตอบ แยกเพื่อจัดการโค้ดให้เป็นระเบียบ, ง่ายต่อการบำรุงรักษา, ลดความซับซ้อน และเพิ่มความสามารถในการ reuse.

2. **CMakeLists.txt Management**: การเพิ่มไฟล์ source ใหม่ต้องแก้ไขอะไรบ้าง? ตอบ เพิ่มไฟล์ source ใหม่ใน CMakeLists.txt โดยการเพิ่มชื่อไฟล์ใน src หรือ add_executable().

3. **Header Files**: บทบาทของไฟล์ .h คืออะไร และทำไมต้องมี? ตอบ ไฟล์ .h ใช้เพื่อประกาศฟังก์ชัน, ตัวแปร, และโครงสร้างที่แชร์ระหว่างหลายไฟล์ source, ช่วยให้การอ้างอิงและการรวมโค้ดง่ายขึ้น.

4. **Include Directories**: เหตุใด CMakeLists.txt ต้องระบุ INCLUDE_DIRS? ตอบ ระบุ INCLUDE_DIRS เพื่อ CMake รู้ว่าไฟล์ header อยู่ที่ไหน, ทำให้สามารถค้นหาและรวมไฟล์ header ได้ถูกต้อง

5. **Git Ignore**: ไฟล์ .gitignore ช่วยอะไรในการจัดการ ESP32 project? ตอบ ป้องกันไม่ให้ไฟล์ที่ไม่จำเป็น (เช่น ไฟล์ build, log) ถูกติดตามใน Git, ช่วยให้ repo สะอาดและเบา.

6. **Task Management**: การใช้ FreeRTOS task ในโมดูล LED ช่วยอะไร?  ตอบ การใช้ FreeRTOS task ช่วยให้การทำงานของโมดูล LED เป็นอิสระ, ทำงานแบบ parallel และลดเวลา response.

7. **Code Organization**: ข้อดีของการแยกโมดูล sensor, display, led เป็นไฟล์แยกคืออะไร? ตอบ แยกโมดูล sensor, display, LED เป็นไฟล์แยกช่วยให้โค้ดอ่านง่าย, บำรุงรักษาง่าย, และรองรับการปรับปรุง/เพิ่มเติมฟีเจอร์ในอนาคต.
...
**สังเกต:**
- การเพิ่มไฟล์ source ส่งผลต่อขนาด binary อย่างไร? ตอบ เพิ่มไฟล์ source จะทำให้ขนาด binary เพิ่มขึ้นตามจำนวนฟังก์ชันและโค้ดที่ถูกคอมไพล์.
- LED กะพริบทุกกี่วินาที? ตอบ LED กะพริบทุก 1 วินาที (จากฟังก์ชัน toggle ที่ถูกเรียกทุกครั้ง)

- แต่ละโมดูลแสดงข้อมูล file และ line อย่างไร?
ตอบ 
๐ display.c: แสดงจาก ./main/display.c, line: [number]

๐ sensor.c: แสดงจาก ./main/sensor.c, line: [number]

๐ led.c: แสดงจาก ./main/led.c, line: [number]
